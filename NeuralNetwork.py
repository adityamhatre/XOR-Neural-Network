...
class NeuralNetwork():
    ...
    def predict(self, inputs, test=False):
        hidden_output = self.sigmoid(np.dot(inputs, self.hidden_layer.weights_from_previous_to_this))
        output = self.sigmoid(np.dot(hidden_output, self.output_layer.weights_from_previous_to_this))
        if test:
            print("--")
            print(inputs)
            print(hidden_output)
